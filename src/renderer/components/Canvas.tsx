...
// Map answer values to lemon expression types
type LemonExpression = 'neutral' | 'yes' | 'no' | 'probably' | 'probably_not' | 'dont_know'

function answerToExpression(answer: AnswerValue | 'neutral'): LemonExpression {
  const map: Record<AnswerValue | 'neutral', LemonExpression> = {
    'neutral': 'neutral',
    'yes': 'yes',
    'no': 'no',
    'probably': 'probably',
...
// Fallback SVG lemonade expressions (used if 3D renders not available)
...
export default function Canvas() {
  ...
  // Check if 3D lemon images are available on mount
  useEffect(() => {
    // Try to load the neutral expression to test if 3D renders exist
    const testImg = new Image()
    testImg.onload = () => setUse3D(true)
    testImg.onerror = () => {
      console.warn('[Canvas] 3D lemon renders not found, using SVG fallback')
      console.warn('[Canvas] Run "npm run setup:lemon" to generate 3D mascot images')
      setUse3D(false)
    }
    testImg.src = '/lemon-assets/lemon-neutral.png'
  }, [])
  
  // Determine which image to show
  const getDisplayImage = (): string => {
    // If we have a final generated image (character portrait), show that
    if (currentImageUrl && (phase === 'revealed' || phase === 'hero_render' || phase === 'guessing')) {
    ...
    // Try 3D render first, fall back to SVG if not available or on error
    if (use3D && !imageError) {
      const lemonExpression = answerToExpression(expression)
      return `/lemon-assets/lemon-${lemonExpression}.png`
    }
  ...
  const handleImageError = () => {
    console.warn('[Canvas] Failed to load 3D lemon image, falling back to SVG')
    setImageError(true)
    setUse3D(false)
  }
