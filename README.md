...
## 🍋 3D Lemon Mascot Setup (Optional)

The game includes a cute 2D SVG lemonade mascot by default. For a more polished look, you can generate **3D rendered lemon character expressions** using SDXL-Turbo!

### Generate 3D Lemon Images

**Prerequisites:**
- Lemonade server running with `sdxl-turbo` model loaded
- ~6GB GPU VRAM (or use CPU fallback, slower)
...
This will generate 6 PNG images (512x512 each) in `public/lemon-assets/`:
- `lemon-neutral.png` - Friendly smiling face
- `lemon-yes.png` - Excited happy face
- `lemon-no.png` - Sad disappointed face
- `lemon-probably.png` - Thoughtful considering face
- `lemon-probably_not.png` - Skeptical uncertain face
- `lemon-dont_know.png` - Confused puzzled face

**Generation takes about 30-60 seconds** (6 images × 5 seconds each).

### What It Looks Like

The 3D lemon character is styled like the Kool-Aid Man:
- 3D rendered glass filled with yellow lemonade
- Visible ice cubes floating inside
- Lemon slice garnish on rim
- Condensation droplets on glass
- Cartoon arms and legs with white gloves and shoes
- Pixar/CGI style rendering
- Different facial expressions for each answer type

### Fallback Behavior

If 3D images aren't generated, the app automatically falls back to the 2D SVG mascot. Both work perfectly! The 3D version is just prettier. 🍋✨

### Regenerate Images

To regenerate with different results:
```bash
rm -rf public/lemon-assets/*.png
npm run setup:lemon
```
